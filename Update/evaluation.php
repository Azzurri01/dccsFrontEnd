...
<div class="menu">
  <nav>
    <ul>
		...
		<form class="Evaluation-form" action="" method="put">
		<h1>Update</h1>
		<table>
			<tr>
				<td>ID:</td>
			...
			<tr>
				<td>Evaluation:</td>
				<td><input type="text" name="evaluation" class="form-control" placeholder="Enter Evaluation"></td>
			</tr>
			<tr>
...
{
	if(!empty($_GET['id']) && !empty($_GET['evaluation']))
	{
  curl_setopt_array($curl, array(
  CURLOPT_PORT => "8080",
  CURLOPT_URL => "http://localhost:8080/evaluation/update",
  ...
  CURLOPT_POSTFIELDS => "{\"id\": \"$id\",\"rating\": \"$evaluation\"}",
...
if ($err) {
  ...
  echo "<p style='margin-left: 660px'>Evaluation updated successfully!!!</p>";
}
	}
